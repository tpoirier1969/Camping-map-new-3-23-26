...
const app={map:null,markerLayer:null,userMarker:null,userAccuracyCircle:null,liveLocationWatchId:null,liveLocationStarted:false,liveLocationLoading:false,liveLocationLastLoadCenter:null,draftMarker:null,baseLayers:{},sites:[],shownSites:[],stateData:{},enabledStates:new Set(),enabledLayers:new Set(),filters:{},draftPoint:null,draftQueue:[],supabase:null,session:null,restRoadsideStats:null,localAreaCenter:null,nearMeActive:false,loadSeq:0,restOnlyMode:false,routeSearch:{active:false,coords:[],basePoints:[],shapePoints:[],bufferMiles:25,layer:null,previousStates:null,distanceMiles:null,durationMinutes:null},miDynamicLoaded:{mdot:false,localTraveler:false,privateRv:false,overnight:false}};
...
function routeHandleInfluenceMiles(){
  const measure=routeMeasure(app.routeSearch.coords||[]);
  const total=Number(measure.total)||0;
  if(total<=0)return 0;
  const preferred=Math.max(8,total*.06);
  return Math.min(35,preferred,Math.max(3,total*.25));
}
function expandedShapeRoutePoints(shape,useWide){
  const handle={lat:Number(shape.lat),lng:Number(shape.lng),seq:clampRouteSeq(shape.seq),kind:'route-handle'};
  if(!useWide)return [handle];
  const coords=app.routeSearch.coords||[];
  const measure=routeMeasure(coords);
  if(coords.length<2||measure.total<=0)return [handle];
  const anchor=pointAtRouteSequence(handle.seq,coords);
  if(!anchor)return [handle];
  const moved=distanceLatLngMiles(anchor.lat,anchor.lng,handle.lat,handle.lng);
  const influence=routeHandleInfluenceMiles();
  if(moved<1.0||influence<3)return [handle];
  const centerMiles=handle.seq*measure.total;
  const beforeSeq=clampRouteSeq((centerMiles-influence)/measure.total);
  const afterSeq=clampRouteSeq((centerMiles+influence)/measure.total);
  if(afterSeq-beforeSeq<.012)return [handle];
  const before=pointAtRouteSequence(beforeSeq,coords);
  const after=pointAtRouteSequence(afterSeq,coords);
  if(!before||!after)return [handle];
  const dLat=handle.lat-anchor.lat;
  const dLng=handle.lng-anchor.lng;
  return [
    {lat:before.lat+dLat,lng:before.lng+dLng,seq:beforeSeq,kind:'route-handle-wide'},
    handle,
    {lat:after.lat+dLat,lng:after.lng+dLng,seq:afterSeq,kind:'route-handle-wide'}
  ];
}
function routePointsWithShapes(useWide=true){
  ...
  const shapes=(app.routeSearch.shapePoints||[])
    ...
    .flatMap(p=>expandedShapeRoutePoints(p,useWide));
...
function addRouteShapePoint(latlng){
  ...
  drawRouteLine();
  notify('Route handle added. Drag it onto the road you want, then release to reroute. The app now gives the move a wider influence when possible. Alt-click a handle to remove it.');
}
async function rerouteWithShapePoints(showNotice){
  const widePoints=routePointsWithShapes(true);
  if(widePoints.length<2)return;
  setLoading(true,'Re-routing…');
  let usedWide=widePoints.length!==routePointsWithShapes(false).length;
  try{
    let route;
    try{
      route=await requestOsrmRoute(widePoints);
    }catch(wideErr){
      const fallbackPoints=routePointsWithShapes(false);
      if(fallbackPoints.length<widePoints.length){
        usedWide=false;
        route=await requestOsrmRoute(fallbackPoints);
      }else{
        throw wideErr;
      }
    }
    ...
    fitRouteView();
    updateRouteStatus();
    if(showNotice)notify(usedWide?'Route reshaped with wider route influence.':'Route reshaped.');
  }catch(err){
    console.error(err);
