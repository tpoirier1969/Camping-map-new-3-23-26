...
function renderAreaOutlineList(){
  ...
  if(count){
    ...
    const extra=boondockingCount?` ${boondockingCount} official boondocking/dispersed rule-area outline${boondockingCount===1?' is':'s are'} available for the selected state${app.enabledStates.size===1?'':'s'}.`:'';
    count.textContent=records.length?`${records.length} official context outlines loaded.${extra} Check any number to show them together.`:'No official context outlines loaded.';
  }
}
