...
const app={map:null,markerLayer:null,userMarker:null,draftMarker:null,baseLayers:{},sites:[],shownSites:[],stateData:{},enabledStates:new Set(),enabledLayers:new Set(),filters:{},draftPoint:null,draftQueue:[],supabase:null,session:null,restRoadsideStats:null,localAreaCenter:null,nearMeActive:false,loadSeq:0,restOnlyMode:false};
...
function toggleRestOnlyMode(){app.restOnlyMode=!app.restOnlyMode;syncRestOnlyToggle();renderMarkers(true);notify(app.restOnlyMode?'Showing only Rest Areas & Roadside Stops for selected states.':'Rest-stop focus off. Restored selected layers.');}
function setAllLayers(on){app.restOnlyMode=false;syncRestOnlyToggle();MAP_LAYERS.forEach(l=>on?app.enabledLayers.add(l.key):app.enabledLayers.delete(l.key)); saveLayers(); $('showPendingLayer').checked=app.enabledLayers.has('pending');renderMarkers(false)}
...
async function applyNearMeLocation(lat,lng){const ll=[lat,lng];app.localAreaCenter=ll;app.nearMeActive=true;const codes=mappedStatesNearLocation(lat,lng);app.enabledStates=new Set(codes);saveJson(STORE.states,[...app.enabledStates]);syncStateControls();app.sites=[];app.shownSites=[];renderMarkers(false);notify(`Loading mapped states within about ${NEAR_ME_RADIUS_MILES} miles: ${codes.join(', ')}`,3500);await loadEnabledStates(false);if(app.nearMeActive&&app.localAreaCenter===ll){showUserMarker(ll);fitNearMeRadius(ll);renderMarkers(false);syncStateControls();}}
function nearMe(){if(!navigator.geolocation)return notify('Location is not available in this browser.');app.nearMeActive=true;app.localAreaCenter=null;app.enabledStates=new Set();saveJson(STORE.states,[]);app.sites=[];app.shownSites=[];renderMarkers(false);syncStateControls();notify('Getting your location for Near Me…');navigator.geolocation.getCurrentPosition(pos=>{applyNearMeLocation(pos.coords.latitude,pos.coords.longitude).catch(e=>{console.error(e);notify('Could not load the nearby map area.')})},()=>notify('Could not get your location. Check browser/site location permission.'),{enableHighAccuracy:false,timeout:12000,maximumAge:300000})}
