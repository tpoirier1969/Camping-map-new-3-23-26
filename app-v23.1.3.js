...
function renderAreaOutlineList(){
  ...
  const records=(app.areaOutline&&Array.isArray(app.areaOutline.standalone)?app.areaOutline.standalone:standaloneAreaOutlineRecords())
    .filter(site=>areaOutlineIsAvailable(areaOutlineCandidate(site)))
  ...
  box.innerHTML=records.length?records.map(site=>`<label class="outline-check"><input type="checkbox" data-outline-id="${esc(site.id)}" ${active[site.id]?'checked':''}><span>${esc(site.stateCode?site.stateCode+' — ':'')}${esc(site.name)}</span></label>`).join(''):'<div class="mini-note">No official context outlines loaded.</div>';
  $$('input[data-outline-id]',box).forEach(input=>{
    input.onchange=()=>toggleAreaOutlineByKey(input.dataset.outlineId,input.checked);
  });
  const count=$('areaOutlineCount');
  if(count)count.textContent=records.length?`${records.length} official context outlines loaded. Check any number to show them together.`:'No official context outlines loaded.';
}
async function showSelectedAreaOutline(){
  const checked=$$('#areaOutlineChecklist input[data-outline-id]:checked').map(i=>i.dataset.outlineId);
  if(!checked.length){notify('Check one or more official outlines first.');return;}
  for(const key of checked)await showAreaOutlineByKey(key,{fit:false});
  fitAreaOutline();
}
...
function syncOutlineCheckboxes(){
  const active=app.areaOutline&&app.areaOutline.active?app.areaOutline.active:{};
  $$('#areaOutlineChecklist input[data-outline-id]').forEach(input=>{input.checked=!!active[input.dataset.outlineId];});
}
...
function clearAreaOutline(){
  ...
  if(app.areaOutline){app.areaOutline.active={};app.areaOutline.layers={};}
...
function fitAreaOutline(){
  ...
  if(b&&b.isValid())app.map.fitBounds(b,{padding:[34,34],animate:false});
}
...
function areaRulesHtml(site,outline){
  ...
  const rules=[
    ...
    ['Camping legality',outline.officialCampingLegality||outline.rulesSummary||site.officialCampingLegality],
    ['Use',outline.recommendedUse||'context-only'],
  ...
  return `<div class="area-rules-popup"><div class="popup-title">${esc(site.name||outline.name||'Official area outline')}</div><div class="popup-notice">Context outline only — not a legal campsite boundary.</div><div class="popup-grid">${rules.map(r=>`<div class="popup-row"><strong>${esc(r[0])}</strong><span>${esc(r[1])}</span></div>`).join('')}</div>${sourceUrl?`<div class="popup-actions"><a class="secondary" target="_blank" rel="noopener" href="${esc(sourceUrl)}">Official source</a></div>`:''}</div>`;
}
function areaOutlineLabelText(site){
  const name=String(site.name||'Area outline').replace(/\s+(County Forest|National Forest)$/,' $1');
  return name;
}
function addAreaOutlineLabel(key,site,outline,group){
  const b=group.getBounds&&group.getBounds();
  if(!(b&&b.isValid()))return null;
  const center=b.getCenter();
  const icon=L.divIcon({
    className:'',
    html:`<button class="area-outline-label" type="button">${esc(areaOutlineLabelText(site))}</button>`,
    iconSize:null,
    iconAnchor:[0,0]
  });
  const marker=L.marker(center,{icon,interactive:true,keyboard:true,title:`Rules for ${site.name||'area outline'}`})
    .bindPopup(areaRulesHtml(site,outline),{maxWidth:360});
...
async function showAreaOutlineByKey(key,opts={}){
  ...
  if(app.areaOutline.layers&&app.areaOutline.layers[key]){updateAreaOutlinePanel();if(opts.fit!==false)fitAreaOutline();return;}
  ...
  try{
    ...
    addAreaOutlineLabel(key,site,outline,group);
