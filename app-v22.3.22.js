...
const ALL_STATES=[{"code":"AL","name":"Alabama"},{"code":"AK","name":"Alaska"},{"code":"AZ","name":"Arizona"},{"code":"AR","name":"Arkansas"},{"code":"CA","name":"California"},{"code":"CO","name":"Colorado"},{"code":"CT","name":"Connecticut"},{"code":"DE","name":"Delaware"},{"code":"FL","name":"Florida"},{"code":"GA","name":"Georgia"},{"code":"HI","name":"Hawaii"},{"code":"ID","name":"Idaho"},{"code":"IL","name":"Illinois"},{"code":"IN","name":"Indiana"},{"code":"IA","name":"Iowa"},{"code":"KS","name":"Kansas"},{"code":"KY","name":"Kentucky"},{"code":"LA","name":"Louisiana"},{"code":"ME","name":"Maine"},{"code":"MD","name":"Maryland"},{"code":"MA","name":"Massachusetts"},{"code":"MI","name":"Michigan"},{"code":"MN","name":"Minnesota"},{"code":"MS","name":"Mississippi"},{"code":"MO","name":"Missouri"},{"code":"MT","name":"Montana"},{"code":"NE","name":"Nebraska"},{"code":"NV","name":"Nevada"},{"code":"NH","name":"New Hampshire"},{"code":"NJ","name":"New Jersey"},{"code":"NM","name":"New Mexico"},{"code":"NY","name":"New York"},{"code":"NC","name":"North Carolina"},{"code":"ND","name":"North Dakota"},{"code":"OH","name":"Ohio"},{"code":"OK","name":"Oklahoma"},{"code":"OR","name":"Oregon"},{"code":"PA","name":"Pennsylvania"},{"code":"RI","name":"Rhode Island"},{"code":"SC","name":"South Carolina"},{"code":"SD","name":"South Dakota"},{"code":"TN","name":"Tennessee"},{"code":"TX","name":"Texas"},{"code":"UT","name":"Utah"},{"code":"VT","name":"Vermont"},{"code":"VA","name":"Virginia"},{"code":"WA","name":"Washington"},{"code":"WV","name":"West Virginia"},{"code":"WI","name":"Wisconsin"},{"code":"WY","name":"Wyoming"}];
  ...
  function firstPresent(obj, keys, fallback=''){ if(!obj) return fallback; for(const key of keys){ if(Object.prototype.hasOwnProperty.call(obj,key) && obj[key]!==undefined && obj[key]!==null && obj[key]!=='' ) return obj[key]; } return fallback; }
  function asBool(value){ if(value===true || value===false) return value; const raw=String(value==null?'':value).trim().toLowerCase(); return raw==='true' || raw==='1' || raw==='yes' || raw==='y' || raw==='t'; }
  function parseReviewFlags(value){ if(Array.isArray(value)) return value; const raw=String(value==null?'':value).trim(); if(!raw) return []; try{ const parsed=JSON.parse(raw); return Array.isArray(parsed)?parsed:[]; }catch{} return raw.split(/\s*[;,|]\s*/).map(v=>v.trim()).filter(Boolean); }
  function compareSitesByStateName(a,b){ const aState=String(a.stateCode||''); const bState=String(b.stateCode||''); if(aState!==bState) return aState.localeCompare(bState); return String(a.name||'').localeCompare(String(b.name||'')); }
  function normalizeDbVerifiedRow(row){ return { id:firstPresent(row,['id']), legacyId:firstPresent(row,['legacyId','legacy_id']), name:firstPresent(row,['name']), lat:Number(firstPresent(row,['lat'])), lng:Number(firstPresent(row,['lng'])), stateCode:firstPresent(row,['stateCode','state_code']), stateName:firstPresent(row,['stateName','state_name']), layer:firstPresent(row,['layer','layer_code']), subtype:firstPresent(row,['subtype','subtype_code'],'unknown'), siteForm:firstPresent(row,['siteForm','site_form','site_form_code'],'campsite'), description:firstPresent(row,['description']), website:firstPresent(row,['website','primary_url']), rawCategory:firstPresent(row,['rawCategory','raw_category']), categoryLabel:firstPresent(row,['categoryLabel','category_label']), sourceFolder:firstPresent(row,['sourceFolder','source_folder']), owner:firstPresent(row,['owner','owner_name']), layerLabel:firstPresent(row,['layerLabel','layer_label']), cost:firstPresent(row,['cost']), costDisplay:firstPresent(row,['costDisplay','cost_display']), costCheckedDate:firstPresent(row,['costCheckedDate','cost_checked_date']), costCheckedYear:firstPresent(row,['costCheckedYear','cost_checked_year','costYear','cost_year']), costSourceUrl:firstPresent(row,['costSourceUrl','cost_source_url']), costSourceName:firstPresent(row,['costSourceName','cost_source_name']), costNeedsReview:asBool(firstPresent(row,['costNeedsReview','cost_needs_review'],false)), costIsFree:asBool(firstPresent(row,['costIsFree','cost_is_free'],false)), showers:firstPresent(row,['showers']), access:firstPresent(row,['access']), amenities:firstPresent(row,['amenities']), trailheads:firstPresent(row,['trailheads']), season:firstPresent(row,['season','seasonNotes','season_notes']), reviewSummary:firstPresent(row,['reviewSummary','review_summary']), rating:firstPresent(row,['rating']), extraLinks:firstPresent(row,['extraLinks','extra_links','sourceLinks','source_links']), imageLinks:firstPresent(row,['imageLinks','image_links']), verificationStatus:firstPresent(row,['verificationStatus','verification_status'],'verified'), confidence:Number(firstPresent(row,['confidence'],0)||0), sourceCount:Number(firstPresent(row,['sourceCount','source_count'],0)||0), pending:false }; }
  function normalizeDbPendingRow(row){ return { id:firstPresent(row,['id']), legacyId:firstPresent(row,['legacyId','legacy_id']), name:firstPresent(row,['name']), lat:Number(firstPresent(row,['lat'])), lng:Number(firstPresent(row,['lng'])), stateCode:firstPresent(row,['stateCode','state_code']), stateName:firstPresent(row,['stateName','state_name']), layer:firstPresent(row,['proposedLayer','proposed_layer','layer','layer_code'],'info'), subtype:firstPresent(row,['proposedSubtype','proposed_subtype','subtype','subtype_code'],'unknown'), siteForm:firstPresent(row,['proposedSiteForm','proposed_site_form','siteForm','site_form','site_form_code'],'campsite'), description:firstPresent(row,['description']), website:firstPresent(row,['website','primary_url']), owner:firstPresent(row,['owner','owner_name']), layerLabel:firstPresent(row,['layerLabel','layer_label'],'Needs verification'), cost:firstPresent(row,['cost']), costDisplay:firstPresent(row,['costDisplay','cost_display']), costCheckedDate:firstPresent(row,['costCheckedDate','cost_checked_date']), costCheckedYear:firstPresent(row,['costCheckedYear','cost_checked_year','costYear','cost_year']), costSourceUrl:firstPresent(row,['costSourceUrl','cost_source_url']), costSourceName:firstPresent(row,['costSourceName','cost_source_name']), costNeedsReview:asBool(firstPresent(row,['costNeedsReview','cost_needs_review'],false)), costIsFree:asBool(firstPresent(row,['costIsFree','cost_is_free'],false)), showers:firstPresent(row,['showers']), access:firstPresent(row,['access']), amenities:firstPresent(row,['amenities']), trailheads:firstPresent(row,['trailheads']), season:firstPresent(row,['season','seasonNotes','season_notes']), reviewSummary:firstPresent(row,['reviewSummary','review_summary']), rating:firstPresent(row,['rating']), extraLinks:firstPresent(row,['extraLinks','extra_links','sourceLinks','source_links']), imageLinks:firstPresent(row,['imageLinks','image_links']), reviewFlags:parseReviewFlags(firstPresent(row,['reviewFlags','review_flags'],[])), reviewNotes:firstPresent(row,['reviewNotes','review_notes']), verificationStatus:'pending', confidence:Number(firstPresent(row,['confidence'],50)||50), sourceCount:Number(firstPresent(row,['sourceCount','source_count'],0)||0), pending:true }; }
  ...
  function showMapLoading(message){
    ...
    clearTimeout(app._loadingHideTimer);
    if(app._loadingHideFrame1) cancelAnimationFrame(app._loadingHideFrame1);
    if(app._loadingHideFrame2) cancelAnimationFrame(app._loadingHideFrame2);
    app._loadingHideFrame1=null;
    app._loadingHideFrame2=null;
    els.mapLoading.classList.remove('hidden');
    els.mapLoading.style.display='flex';
  ...
  function hideMapLoading(){
    ...
    const applyHide=()=>{
      ...
      if(app.map){
        try{ app.map.invalidateSize(false); }catch(_e){}
      }
    };
    clearTimeout(app._loadingHideTimer);
    if(app._loadingHideFrame1) cancelAnimationFrame(app._loadingHideFrame1);
    if(app._loadingHideFrame2) cancelAnimationFrame(app._loadingHideFrame2);
    app._loadingHideFrame1=requestAnimationFrame(()=>{
      app._loadingHideFrame2=requestAnimationFrame(()=>{
        app._loadingHideFrame1=null;
        app._loadingHideFrame2=null;
        applyHide();
      });
    });
    app._loadingHideTimer=setTimeout(()=>{
      if(app._loadingHideFrame1) cancelAnimationFrame(app._loadingHideFrame1);
      if(app._loadingHideFrame2) cancelAnimationFrame(app._loadingHideFrame2);
      app._loadingHideFrame1=null;
      app._loadingHideFrame2=null;
  ...
  function failBoot(message){try{els.statusLine.textContent=message||'Map failed to start.'; hideMapLoading(); notify(message||'Map failed to start.',5000)}catch(e){console.error(e)}}
...
function buildLayerControls(){els.layerList.innerHTML=layerDefs.map(def=>`<label class="check small"><input type="checkbox" data-layer-toggle="${def.key}" ${app.enabledLayers.has(def.key)?'checked':''}><span class="legend-dot ${def.css}"></span><span>${escapeHtml(def.label)}</span></label>`).join(''); els.layerList.querySelectorAll('[data-layer-toggle]').forEach(i=>i.addEventListener('change',()=>{const k=i.getAttribute('data-layer-toggle'); i.checked?app.enabledLayers.add(k):app.enabledLayers.delete(k); if(k==='pending'){localStorage.setItem(STORAGE_KEYS.showPending, app.enabledLayers.has('pending')?'1':'0'); els.showPendingLayer.checked=app.enabledLayers.has('pending'); updatePendingMeta();} saveSet(STORAGE_KEYS.enabledLayers,app.enabledLayers); syncQuickFilterButtons(); queueRender(false);})); syncQuickFilterButtons(); updatePendingMeta();}
  ...
  function initMap(){app.map=L.map('map',{zoomControl:true,preferCanvas:true}).setView([44.9,-89.7],6); app.baseLayerDefs={
    ...
    app.map.whenReady(()=>{ hideMapLoading(); });
    app.map.on('resize',()=>{ if(app.allSelectedSites.length || app.didInitialFit) hideMapLoading(); });
    mountDesktopLegend();
  }
  ...
  function setBasemap(key){const chosen=app.baseLayerDefs[key]?key:'topo'; app.basemapKey=chosen; clearBaseLayers(); app.baseLayerDefs[chosen].forEach(layer=>{ if(layer && typeof layer.on==='function'){ layer.on('load', hideMapLoading); layer.on('tileload', hideMapLoading); } layer.addTo(app.map); app.activeBaseLayers.push(layer)}); els.basemapSelect.value=chosen; localStorage.setItem(STORAGE_KEYS.basemap,chosen)}
  ...
  function mapDbVerified(row){ return normalizeDbVerifiedRow(row); }
  function mapDbPending(row){ return normalizeDbPendingRow(row); }
  function supabaseDbCandidates(){
    const client=getSupabaseClient();
    if(!client) return [];
    const configuredSchema=(window.CAMPING_SUPABASE_CONFIG&&window.CAMPING_SUPABASE_CONFIG.schema)||'camping';
    const out=[];
    const seen=new Set();
    function add(label, db){ if(!db || seen.has(label)) return; seen.add(label); out.push({label,db}); }
    if(typeof client.schema==='function'){
      add(`schema:${configuredSchema}`, client.schema(configuredSchema));
      if(configuredSchema!=='public') add('schema:public', client.schema('public'));
    }
    add('default', client);
    return out;
  }
  async function runSupabaseQuery(query){
    try{ const {data,error}=await query; return {data:data||[], error:error||null}; }
    catch(error){ return {data:[], error}; }
  }
  async function fetchSupabaseBatch(db, mode){
    const verifiedQuery = mode==='views'
      ? db.from('verified_sites_v').select('*')
      : db.from('sites').select('*').eq('verification_status','verified');
    const pendingQuery = mode==='views'
      ? db.from('pending_sites_v').select('*')
      : db.from('sites').select('*').eq('verification_status','pending');
    const [verifiedRes,pendingRes]=await Promise.all([runSupabaseQuery(verifiedQuery), runSupabaseQuery(pendingQuery)]);
    return {
      verified:(verifiedRes.data||[]).map(mapDbVerified).sort(compareSitesByStateName),
      pending:(pendingRes.data||[]).map(mapDbPending).sort(compareSitesByStateName),
      verifiedError:verifiedRes.error,
      pendingError:pendingRes.error
    };
  }
  async function fetchSupabaseSitesRobust(){
    const attempts=[];
    for(const candidate of supabaseDbCandidates()){
      for(const mode of ['views','table']){
        const result=await fetchSupabaseBatch(candidate.db, mode);
        attempts.push({label:`${candidate.label}/${mode}`, verifiedError:result.verifiedError, pendingError:result.pendingError, verifiedCount:result.verified.length, pendingCount:result.pending.length});
        if(!result.verifiedError && !result.pendingError && (result.verified.length || result.pending.length)){
          return {...result, source:`${candidate.label}/${mode}`, usedLocalPending:false};
        }
        if(!result.verifiedError && result.verified.length){
          return {...result, source:`${candidate.label}/${mode}`, usedLocalPending:true, warning:result.pendingError};
        }
        if(!result.verifiedError && !result.pendingError){
          return {...result, source:`${candidate.label}/${mode}`, usedLocalPending:false};
        }
      }
    }
    const detail=attempts.map(a=>`${a.label}: verified=${a.verifiedError?(a.verifiedError.message||a.verifiedError.details||'error'):'ok'} (${a.verifiedCount}), pending=${a.pendingError?(a.pendingError.message||a.pendingError.details||'error'):'ok'} (${a.pendingCount})`).join(' | ');
    throw new Error(detail||'Unknown Supabase load error');
  }
  async function loadSupabaseData(applyFit){
    if(!app.supabaseSession){ return; }
    showMapLoading('Loading Supabase data…');
    try{
      const result=await fetchSupabaseSitesRobust();
      app.supabaseSitesVerified=result.verified||[];
      app.supabaseSitesPending=result.pending||[];
      if(result.usedLocalPending){
        await loadLocalPending();
        app.supabaseSitesPending=(app.localPendingSites||[]).slice();
        notify('Supabase verified sites loaded. Pending layer is temporarily using local pending data.',5000);
      }
  ...
  function formatCostDisplay(site){
    if(!site || site.layer==='boondocking' || site.costIsFree) return '';
    const display=textValue(site.costDisplay);
    if(isUsefulText(display)) return display;
    const raw=textValue(site.cost);
    if(!isUsefulText(raw)) return '';
    const checked=textValue(site.costCheckedDate);
    const year=site.costCheckedYear||site.costYear||'';
    if(checked && /^\d{4}-\d{2}/.test(checked) && !/^In [A-Z][a-z]+ 20\d{2},/.test(raw)) {
      const [yy,mm]=checked.split('-');
      const monthNames=['January','February','March','April','May','June','July','August','September','October','November','December'];
      const monthLabel=monthNames[Math.max(0,Math.min(11,Number(mm)-1))] || checked;
      return `In ${monthLabel} ${yy}, ${raw.charAt(0).toLowerCase()+raw.slice(1)}`;
    }
  ...
  function popupHtml(site){
    ...
    pushRow('Cost source', site.costSourceName);
  ...
  window.addEventListener('load',()=>{
    setTimeout(()=>{
      ...
      if(app.allSelectedSites.length || app.didInitialFit || app.loadedStates.size){
        hideMapLoading();
      }
    },0);
  });
