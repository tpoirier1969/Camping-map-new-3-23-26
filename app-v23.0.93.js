...
const STORE={states:'campingMap.enabledStates.v22328',layers:'campingMap.layers.v22328',basemap:'campingMap.basemap.v22328',queue:'campingMap.draftQueue.v22328',filters:'campingMap.filters.v22328',pending:'campingMap.showPending.v22328',desktopMode:'campingMap.desktopMode.v23087',savedRoutes:'campingMap.savedRoutes.v23093'};
...
function getSavedRoutes(){
  const list=readJson(STORE.savedRoutes,[]);
  return Array.isArray(list)?list.filter(r=>r&&r.id&&r.name):[];
}
function saveSavedRoutes(list){
  saveJson(STORE.savedRoutes,(Array.isArray(list)?list:[]).slice(0,60));
  renderSavedRoutes();
}
function renderSavedRoutes(){
  ...
  sel.innerHTML='<option value="">Saved routes</option>'+routes.map(r=>`<option value="${esc(r.id)}">${esc(r.name)}</option>`).join('');
...
function saveCurrentRoute(){
  ...
  const existing=routes.findIndex(r=>String(r.name||'').toLowerCase()===name.toLowerCase());
  if(existing>=0)routes.splice(existing,1,payload); else routes.unshift(payload);
  saveSavedRoutes(routes);
  const sel=$('routeSavedSelect');
  if(sel)sel.value=payload.id;
  notify(`Saved route: ${name}`);
}
async function loadSavedRoute(){
  const id=$('routeSavedSelect')?.value;
  if(!id){notify('Choose a saved route first.');return;}
  const saved=getSavedRoutes().find(r=>r.id===id);
  if(!saved){notify('Saved route not found.');renderSavedRoutes();return;}
  ...
  setLoading(true,'Loading saved route…');
  try{
    ...
    fitRouteView();
    updateRouteStatus();
    notify(`Loaded route: ${saved.name}`);
  }catch(err){console.error(err);notify(err&&err.message?err.message:'Could not load saved route.');}
  finally{setLoading(false);if(btn)btn.disabled=false;}
}
function deleteSavedRoute(){
  const id=$('routeSavedSelect')?.value;
  if(!id){notify('Choose a saved route to delete.');return;}
  const routes=getSavedRoutes();
  const route=routes.find(r=>r.id===id);
  saveSavedRoutes(routes.filter(r=>r.id!==id));
  notify(route?`Deleted saved route: ${route.name}`:'Saved route deleted.');
}
