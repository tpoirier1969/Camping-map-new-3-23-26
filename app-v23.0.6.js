...
const LAYERS=[
 ...
 {key:'rest-truck',label:'Rest areas / truck stops (check overnight legality)',css:'pin-rest',icon:ICONS.parking},
...
const app={map:null,markerLayer:null,userMarker:null,draftMarker:null,baseLayers:{},sites:[],shownSites:[],stateData:{},enabledStates:new Set(),enabledLayers:new Set(),filters:{},draftPoint:null,draftQueue:[],supabase:null,session:null};
...
function layerKey(site){let layer=String(site.layer||site.proposedLayer||'').toLowerCase();let subtype=String(site.subtype||site.proposedSubtype||'').toLowerCase();let form=String(site.siteForm||site.proposedSiteForm||'').toLowerCase();let raw=(layer+' '+subtype+' '+form+' '+(site.layerLabel||'')+' '+(site.categoryLabel||'')).toLowerCase(); if(site.pending||site.verificationStatus==='pending')return 'pending'; if(/rest|truck/.test(raw))return 'rest-truck'; if(/overnight|parking|walmart|cracker|cabela|casino/.test(raw))return 'overnight-parking'; if(layer==='boondocking'||/dispersed|boondock/.test(raw))return 'boondocking'; if(layer==='info'||/reference/.test(raw))return 'info'; if(layer==='private')return 'private'; if(['state','federal','local'].includes(layer)&&subtype==='rustic')return 'rustic'; if(['state','federal','local'].includes(layer)&&subtype==='modern')return 'modern'; if(subtype==='rustic')return 'rustic'; if(subtype==='modern')return 'modern'; return 'info';}
...
function mappedStateEntries(){return manifestEntries().filter(s=>Number(s.count||0)>0 || s.file)}
...
function selectedStateSummary(){const n=app.enabledStates.size; const mapped=mappedStateEntries().length; if(n===1)return `${stateLabel([...app.enabledStates][0])} selected`; if(n===mapped)return `All ${mapped} mapped states`; return `${n} states selected`;}
function syncStateControls(){$$('[data-state-code]').forEach(cb=>{cb.checked=app.enabledStates.has(cb.dataset.stateCode)}); const summary=$('stateSelectionSummary'); if(summary)summary.textContent=selectedStateSummary(); const note=$('stateSelectionNote'); if(note){const n=app.enabledStates.size; note.textContent=n===1?`${stateLabel([...app.enabledStates][0])} selected.`:`${n} states selected. The map will zoom to the combined selected area.`;}}
function stateLabel(code){const row=manifestEntries().find(s=>s.code===code); return row?(row.name||row.code):code}
function setEnabledStates(codes,fit=true){const valid=new Set(mappedStateEntries().map(s=>s.code)); const picked=(codes||[]).filter(c=>valid.has(c)); app.enabledStates=new Set(picked.length?picked:[DEFAULT_STATE]); saveJson(STORE.states,[...app.enabledStates]); syncStateControls(); loadEnabledStates(fit)}
...
function setAllLayers(on){MAP_LAYERS.forEach(l=>on?app.enabledLayers.add(l.key):app.enabledLayers.delete(l.key)); if(!on){app.enabledLayers.add('modern');app.enabledLayers.add('rustic')} saveLayers(); $('showPendingLayer').checked=app.enabledLayers.has('pending');renderMarkers(false)}
...
function syncFilters(){const f=app.filters||{};$('costFilter').value=f.maxCost||'';$('accessFilter').value=f.access||'';$$('[data-filter-chip]').forEach(c=>c.classList.toggle('active',!!(f.chips&&f.chips[c.dataset.filterChip])))}
function saveFilters(){saveJson(STORE.filters,app.filters)}
...
async function loadEnabledStates(fit){setLoading(true,'Loading map…'); const states=[...app.enabledStates]; app.sites=[]; for(const code of states){await loadState(code); app.sites.push(...(app.stateData[code]||[]));} if(app.enabledLayers.has('pending')) app.sites.push(...getPendingSites()); renderMarkers(fit); setLoading(false);}
function loadState(code){if(app.stateData[code])return Promise.resolve(); const row=manifestEntries().find(s=>s.code===code); if(!row||!row.file){app.stateData[code]=[];return Promise.resolve()} return new Promise(res=>{window.CAMPING_STATE_DATA=window.CAMPING_STATE_DATA||{}; const existing=document.querySelector(`script[data-state-file="${code}"]`); if(existing){app.stateData[code]=window.CAMPING_STATE_DATA[code]||[];return res()} const s=document.createElement('script');s.src=row.file;s.dataset.stateFile=code;s.onload=()=>{app.stateData[code]=(window.CAMPING_STATE_DATA&&window.CAMPING_STATE_DATA[code])||[];res()};s.onerror=()=>{app.stateData[code]=[];res()};document.head.appendChild(s)})}
...
function markerIcon(site){const d=layerDef(layerKey(site));return L.divIcon({className:'',html:`<span class="map-pin ${d.css}">${d.icon}</span>`,iconSize:[24,24],iconAnchor:[12,12],popupAnchor:[0,-12]})}
