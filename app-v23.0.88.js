...
function updateRouteStatus(){
  ...
  if(shapes)bits.push(`${shapes} route handle${shapes===1?'':'s'}`);
...
function nearestRouteSequence(latlng){
  ...
  return Math.max(.001,Math.min(.999,bestIndex/(coords.length-1)));
}
function routePointsWithShapes(){
  ...
  const shapes=(app.routeSearch.shapePoints||[]).map(p=>Object.assign({},p,{kind:'route-handle'})).filter(p=>Number.isFinite(Number(p.lat))&&Number.isFinite(Number(p.lng)));
...
function addRouteShapePoint(latlng){
  ...
  drawRouteLine();
  notify('Route handle added. Drag it onto the road you want, then release to reroute. Alt-click a handle to remove it.');
}
async function rerouteWithShapePoints(showNotice){
  ...
  try{
    ...
    fitRouteView();
    updateRouteStatus();
    if(showNotice)notify('Route reshaped.');
  }catch(err){
    console.error(err);
