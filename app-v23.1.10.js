...
function renderAreaOutlineList(){
  ...
  const selectedCodes=sortStateCodes([...app.enabledStates]);
  const nested=selectedCodes.length>2;
  const itemHtml=site=>`<label class="outline-check"><input type="checkbox" data-outline-id="${esc(site.id)}" ${active[site.id]?'checked':''}><span>${esc(site.name)}</span></label>`;
  if(!records.length){
    ...
    box.innerHTML=selectedCodes.filter(code=>byState[code]&&byState[code].length).map(code=>`<details class="outline-state-group"><summary>${esc(stateLabel(code))} <span>${byState[code].length}</span></summary><div class="outline-state-items">${byState[code].map(itemHtml).join('')}</div></details>`).join('');
  }else{
    box.innerHTML=selectedCodes.filter(code=>byState[code]&&byState[code].length).map(code=>`<div class="outline-state-heading">${esc(stateLabel(code))}</div>${byState[code].map(itemHtml).join('')}`).join('');
  }
  ...
  if(count){
    ...
    count.textContent=records.length?`${records.length} official context outline${records.length===1?'':'s'} listed for the selected state${app.enabledStates.size===1?'':'s'}.${extra} Open the menu and check any number to show them together.`:'No official context outlines listed for the selected state selection.';
  }
  const summary=$('areaOutlineSelectionSummary');
  if(summary){
    const activeSelected=records.filter(site=>active[site.id]).length;
    summary.textContent=activeSelected?`${activeSelected} selected`:(records.length?`${records.length} available`:'None available');
  }
}
