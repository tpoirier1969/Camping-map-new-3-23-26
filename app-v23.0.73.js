...
function clearNearMeMode(){app.nearMeActive=false;app.localAreaCenter=null;app.liveLocationLoading=false;app.liveLocationLastLoadCenter=null;if(app.userAccuracyCircle){app.userAccuracyCircle.remove();app.userAccuracyCircle=null;}if(app.userMarker){app.userMarker.remove();app.userMarker=null;}}
...
const LIVE_LOCATION_AUTO_START=true;
...
function showUserMarker(ll,accuracyMeters){
  ...
  const icon=L.divIcon({className:'',html:`<span class="map-pin pin-user">${ICONS.dot}</span>`,iconSize:[24,24],iconAnchor:[12,12]});
...
async function applyNearMeLocation(lat,lng,accuracyMeters,options={}){
  ...
  if(shouldReload&&!app.liveLocationLoading){
    ...
    notify(`Live location: loading sites within ${NEAR_ME_RADIUS_MILES} miles.`,3500);
...
function startLiveLocation(force=false){
  ...
  if(app.liveLocationStarted&&!force)return;
  ...
  renderMarkers(false);
  syncStateControls();
  setLoading(true,'Getting your live location…');
  notify('Getting your live location…');
  app.liveLocationWatchId=navigator.geolocation.watchPosition(pos=>{
    const c=pos.coords||{};
    applyNearMeLocation(c.latitude,c.longitude,c.accuracy,{forceReload:!app.liveLocationLastLoadCenter}).catch(e=>{console.error(e);setLoading(false);notify('Could not load the local map area.')});
  },err=>{
    app.liveLocationStarted=false;
    app.liveLocationWatchId=null;
    setLoading(false);
    const msg=err&&err.code===1?'Location permission was denied. Allow location for this site to use live follow.':'Could not get your live location. Check browser/site location permission.';
    notify(msg,6000);
  },{enableHighAccuracy:true,timeout:15000,maximumAge:15000});
}
function nearMe(){startLiveLocation(true)}
...
function boot(){initState();initMap();buildControls();initSupabase().catch(()=>{});loadEnabledStates(true).catch(e=>{console.error(e);setLoading(false);notify('Map load failed.');});checkForAppUpdate();if(LIVE_LOCATION_AUTO_START)setTimeout(()=>startLiveLocation(false),650);}
