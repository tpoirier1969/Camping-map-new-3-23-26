...
async function applyNearMeLocation(lat,lng){const ll=[lat,lng];app.localAreaCenter=ll;app.nearMeActive=true;const codes=mappedStatesNearLocation(lat,lng);app.enabledStates=new Set(codes);saveJson(STORE.states,[...app.enabledStates]);syncStateControls();notify(`Loading mapped states within about ${NEAR_ME_RADIUS_MILES} miles: ${codes.join(', ')}`,3500);await loadEnabledStates(false);if(app.nearMeActive&&app.localAreaCenter===ll){showUserMarker(ll);fitNearMeRadius(ll)}}
