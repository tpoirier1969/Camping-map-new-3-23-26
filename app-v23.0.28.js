...
const app={map:null,markerLayer:null,userMarker:null,draftMarker:null,baseLayers:{},sites:[],shownSites:[],stateData:{},enabledStates:new Set(),enabledLayers:new Set(),filters:{},draftPoint:null,draftQueue:[],supabase:null,session:null,restRoadsideStats:null,localAreaCenter:null};
...
function showUpdateNotice(remoteVersion){
  ...
  if(btn)btn.onclick=()=>{try{localStorage.setItem('campingMap.lastManualRefresh',String(Date.now()))}catch(_e){} const path=window.location.pathname||'/'; window.location.href=path+'?refresh='+Date.now();};
}
