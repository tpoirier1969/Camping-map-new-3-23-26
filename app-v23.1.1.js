...
function renderAreaOutlineList(){
  ...
  const current=sel.value;
  sel.innerHTML='<option value="">Choose an official outline…</option>'+records.map(site=>`<option value="${esc(site.id)}">${esc(site.stateCode?site.stateCode+' — ':'')}${esc(site.name)}</option>`).join('');
  if(current&&records.some(site=>site.id===current))sel.value=current;
  const count=$('areaOutlineCount');
  if(count)count.textContent=records.length?`${records.length} official context outlines loaded.`:'No official context outlines loaded.';
}
async function showSelectedAreaOutline(){
  const sel=$('areaOutlineSelect');
  const key=sel&&sel.value;
  if(!key){notify('Choose an official outline first.');return;}
  await showAreaOutlineByKey(key);
}

function updateAreaOutlinePanel(){
  const el=$('areaOutlineStatus');
  if(!el)return;
  const active=app.areaOutline&&app.areaOutline.active;
  if(!active){el.textContent='No official area outline is currently shown.';return;}
  el.innerHTML=`Showing: <strong>${esc(active.name)}</strong><br><span>${esc(active.boundaryRepresents||'Official context boundary')}</span><br><span>${esc(active.caution||'Context outline only — not a legal campsite boundary.')}</span>`;
}
function clearAreaOutline(){
  if(app.areaOutline&&app.areaOutline.layer)app.areaOutline.layer.clearLayers();
  if(app.areaOutline)app.areaOutline.active=null;
  updateAreaOutlinePanel();
  notify('Area outline cleared.');
}
...
async function showAreaOutlineByKey(key){
  ...
  try{
    ...
    if(app.areaOutline.layer)app.areaOutline.layer.clearLayers();
    ...
    app.areaOutline.active={name:site.name||'Area outline',boundaryRepresents:outline.boundaryRepresents,caution:outline.caution,sourceUrl:outline.sourceUrl||areaOutlineFetchUrl(outline)};
    updateAreaOutlinePanel();
    fitAreaOutline();
