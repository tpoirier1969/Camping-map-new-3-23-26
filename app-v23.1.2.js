...
function updateAreaOutlinePanel(){
  ...
  el.innerHTML=`Showing ${active.length} official context outline${active.length===1?'':'s'}:<br>`+
    active.map(o=>`<strong>${esc(o.name)}</strong><br><span>${esc(o.boundaryRepresents||'Official context boundary')}</span><br><span>${esc(o.caution||'Context outline only — not a legal campsite boundary.')}</span>`).join('<hr class="outline-divider">');
...
async function showAreaOutlineByKey(key,opts={}){
  ...
  try{
    ...
    app.areaOutline.active[key]={name:site.name||'Area outline',boundaryRepresents:outline.boundaryRepresents,caution:outline.caution,sourceUrl:outline.sourceUrl||areaOutlineFetchUrl(outline)};
    updateAreaOutlinePanel();
