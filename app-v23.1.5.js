...
function labelRingsForOutline(geo){
  const parts=polygonPartsFromGeoJson(geo).map(ring=>({ring,area:areaSqMilesForRing(ring),point:polygonLabelPoint(ring)})).filter(p=>p.point&&p.area>0.05);
