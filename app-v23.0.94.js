...
const app={map:null,markerLayer:null,userMarker:null,userAccuracyCircle:null,liveLocationWatchId:null,liveLocationStarted:false,liveLocationLoading:false,liveLocationLastLoadCenter:null,draftMarker:null,baseLayers:{},sites:[],shownSites:[],stateData:{},enabledStates:new Set(),enabledLayers:new Set(),filters:{},draftPoint:null,draftQueue:[],supabase:null,session:null,restRoadsideStats:null,localAreaCenter:null,nearMeActive:false,loadSeq:0,restOnlyMode:false,routeSearch:{active:false,coords:[],basePoints:[],shapePoints:[],bufferMiles:25,layer:null,previousStates:null,distanceMiles:null,durationMinutes:null},savedRoutes:[],savedRoutesLoaded:false,savedRoutesError:null,miDynamicLoaded:{mdot:false,localTraveler:false,privateRv:false,overnight:false}};
...
async function saveCurrentRoute(){
  ...
  if(!app.session)return notify('Sign in to Supabase before saving routes across devices.',6000);
...
async function initSupabase(){const cfg=window.CAMPING_SUPABASE_CONFIG;if(!cfg||!window.supabase){$('supabaseStatus').textContent='Static fallback mode.';renderSavedRoutes();return}app.supabase=window.supabase.createClient(cfg.url,cfg.anonKey,{db:{schema:cfg.schema||'public'}});const {data}=await app.supabase.auth.getSession();app.session=data.session||null;$('supabaseStatus').textContent=app.session?'Signed in.':'Supabase ready; not signed in.';await refreshSavedRoutes(false);}
async function signIn(e){e.preventDefault();if(!app.supabase)return notify('Supabase config is not loaded.');const email=$('sbEmail').value.trim(),password=$('sbPassword').value;const {error}=await app.supabase.auth.signInWithPassword({email,password});if(error)return notify(error.message,5000);app.session=(await app.supabase.auth.getSession()).data.session;$('supabaseStatus').textContent='Signed in.';await refreshSavedRoutes(false);}
async function signOut(){if(!app.supabase)return;await app.supabase.auth.signOut();app.session=null;app.savedRoutes=[];app.savedRoutesLoaded=false;app.savedRoutesError=null;$('supabaseStatus').textContent='Signed out.';renderSavedRoutes();}
