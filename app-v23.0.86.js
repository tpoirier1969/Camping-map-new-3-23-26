...
const app={map:null,markerLayer:null,userMarker:null,userAccuracyCircle:null,liveLocationWatchId:null,liveLocationStarted:false,liveLocationLoading:false,liveLocationLastLoadCenter:null,draftMarker:null,baseLayers:{},sites:[],shownSites:[],stateData:{},enabledStates:new Set(),enabledLayers:new Set(),filters:{},draftPoint:null,draftQueue:[],supabase:null,session:null,restRoadsideStats:null,localAreaCenter:null,nearMeActive:false,loadSeq:0,restOnlyMode:false,routeSearch:{active:false,coords:[],bufferMiles:25,layer:null,previousStates:null,distanceMiles:null,durationMinutes:null},miDynamicLoaded:{mdot:false,localTraveler:false,privateRv:false,overnight:false}};
...
function buildControls(){buildStateSelect(); buildLayerList(); buildLegend(); syncFilters(); bindEvents(); updatePendingMeta(); syncRouteControls();}
...
function syncRouteControls(){
  const miles=$('routeMiles');
  if(miles)miles.value=String(app.routeSearch.bufferMiles||25);
  updateRouteStatus();
}
function updateRouteStatus(){
  const el=$('routeStatus');
  if(!el)return;
  if(!app.routeSearch.active){el.textContent='Route Search is off.';return;}
  const miles=app.routeSearch.bufferMiles||25;
  const dist=Number(app.routeSearch.distanceMiles);
  const dur=Number(app.routeSearch.durationMinutes);
  const bits=[`Filtering loaded sites within ${miles} miles of route`];
  if(Number.isFinite(dist))bits.push(`${Math.round(dist)} route miles`);
  if(Number.isFinite(dur))bits.push(`${Math.round(dur/60)} hr ${Math.round(dur%60)} min OSRM estimate`);
  el.textContent=bits.join(' · ')+'.';
}
function lngLatString(points){return points.map(p=>`${p.lng.toFixed(6)},${p.lat.toFixed(6)}`).join(';');}
...
function drawRouteLine(){
  ...
  const line=L.polyline(coords.map(p=>[p.lat,p.lng]),{color:'#246ad4',weight:5,opacity:.82,lineCap:'round',lineJoin:'round',interactive:false});
...
async function runRouteSearch(){
  ...
  const miles=Number($('routeMiles')?.value||25)||25;
