...
const ICONS={tent:'<svg viewBox="0 0 24 24"><path fill="currentColor" d="M3 18.5 10.8 5h2.4L21 18.5h-3.1l-5.1-9.1-2.9 4.9 1.9 4.2H9.2l-1.5-3.2-1.7 3.2H3Zm6.8 0h4.5l-2.2-5-2.3 5Z"/></svg>',tree:'<svg viewBox="0 0 24 24"><path fill="currentColor" d="m12 2 4 5h-2.1l3.6 4.5H15l3 3.8h-4.2V22h-3.6v-6.7H6l3-3.8H6.5L10.1 7H8L12 2Z"/></svg>',mountains:'<svg viewBox="0 0 24 24"><path fill="currentColor" d="M3 19.5 9.4 9.2l2.1 3.2 2.5-4.2L21 19.5h-5.2l-2.5-4-1.9 3.1H3Zm9.2 0h2.1l-1-1.7-1.1 1.7Z"/></svg>',camper:'<svg viewBox="0 0 24 24"><path fill="currentColor" d="M4 8.5h10.6c.8 0 1.6.4 2.1 1l2.3 2.6H21a1 1 0 0 1 1 1v4.9h-1.7a2.6 2.6 0 0 1-5.1 0H9.8a2.6 2.6 0 0 1-5.1 0H3v-8.5a1 1 0 0 1 1-1Zm1.2 1.8v2.8H14v-2.8H5.2Zm11 3.1h3.2l-1.6-1.8a1.1 1.1 0 0 0-.8-.4h-.8v2.2ZM7.2 19a1.2 1.2 0 1 0 0-2.5 1.2 1.2 0 0 0 0 2.4Zm10.6 0a1.2 1.2 0 1 0 0-2.5 1.2 1.2 0 0 0 0 2.4Z"/></svg>',parking:'<svg viewBox="0 0 24 24"><path fill="currentColor" d="M6 3h7.4c3 0 5.1 2.1 5.1 5s-2.1 5.1-5.1 5.1H9.6V21H6V3Zm3.6 3.2v3.7h3.4c1.1 0 1.9-.7 1.9-1.9s-.8-1.8-1.9-1.8H9.6Z"/></svg>',paddle:'<svg viewBox="0 0 24 24"><path fill="currentColor" d="M4.1 18.9 18.9 4.1l1 1L5.1 19.9l-1-1Zm2.4-1.1 1.8-1.8 2.9 2.9c.8.8.8 2.1 0 2.9s-2.1.8-2.9 0l-1.8-1.8c-.6-.6-.6-1.6 0-2.2ZM15.8 5.1l1.9-1.9c.8-.8 2.1-.8 2.9 0l.2.2c.8.8.8 2.1 0 2.9l-1.9 1.9-3.1-3.1ZM4 10.5c2.8 0 5.3.9 7.3 2.5l-1.5 1.5C8.1 13.3 6.1 12.6 4 12.6v-2.1Zm16 1.1v2.1c-2.1 0-4.1.7-5.8 1.9L12.7 14c2-1.6 4.5-2.4 7.3-2.4Z"/></svg>',info:'<svg viewBox="0 0 24 24"><path fill="currentColor" d="M12 2.8A9.2 9.2 0 1 1 2.8 12 9.2 9.2 0 0 1 12 2.8Zm0 4a1.3 1.3 0 1 0 0 2.6 1.3 1.3 0 0 0 0-2.6Zm-1.7 5v1.8h1.1v3.6h-1.1V19h4.5v-1.8h-1.1v-5.4h-3.4Z"/></svg>',draft:'<svg viewBox="0 0 24 24"><path fill="currentColor" d="M4 17.5V20h2.5l8.6-8.6-2.5-2.5L4 17.5Zm12.4-9.9 1.5-1.5a1.2 1.2 0 0 1 1.7 0l.8.8a1.2 1.2 0 0 1 0 1.7l-1.5 1.5-2.5-2.5Z"/></svg>',dot:'<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="6" fill="currentColor"/></svg>'};
const LAYERS=[
 ...
 {key:'boondocking',label:'Boondocking / dispersed',css:'pin-boondocking',icon:ICONS.mountains},
 {key:'boat-backpack',label:'Boat / Backpack',css:'pin-boat-backpack',icon:ICONS.paddle},
...
const app={map:null,markerLayer:null,userMarker:null,draftMarker:null,baseLayers:{},sites:[],shownSites:[],stateData:{},enabledStates:new Set(),enabledLayers:new Set(),filters:{},draftPoint:null,draftQueue:[],supabase:null,session:null,restRoadsideStats:null,localAreaCenter:null,nearMeActive:false,loadSeq:0,restOnlyMode:false,miDynamicLoaded:{mdot:false,localTraveler:false,privateRv:false,overnight:false}};
...
function blankFilters(){return {maxCost:'',access:'',chips:{showers:false}};}
...
function syncStateControls(){$$('[data-state-code]').forEach(cb=>{cb.checked=app.enabledStates.has(cb.dataset.stateCode)}); const summary=$('stateSelectionSummary'); if(summary)summary.textContent=selectedStateSummary(); const note=$('stateSelectionNote'); if(note){const n=app.enabledStates.size; if(app.nearMeActive){const codes=nearMeVisibleStateCodes(); const names=codes.map(stateLabel).join(', '); note.textContent=app.localAreaCenter?(codes.length?`Near Me is active: showing sites within ${NEAR_ME_RADIUS_MILES} miles. Loaded states in range: ${names}.`:`Near Me is active: showing sites within ${NEAR_ME_RADIUS_MILES} miles.`):'Near Me is getting your location…';}else{note.textContent=n===0?'No states selected. Choose one or more states to load map data.':(n===1?`${stateLabel([...app.enabledStates][0])} selected.`:`${n} states selected. The map will zoom to the combined selected area.`);}}}
function stateLabel(code){const row=manifestEntries().find(s=>s.code===code); return row?(row.name||row.code):code}
function clearNearMeMode(){app.nearMeActive=false;app.localAreaCenter=null;if(app.userMarker){app.userMarker.remove();app.userMarker=null;}}
function setEnabledStates(codes,fit=true){clearNearMeMode();const valid=new Set(mappedStateEntries().map(s=>s.code)); const picked=(codes||[]).filter(c=>valid.has(c)); app.enabledStates=new Set(picked); saveJson(STORE.states,[...app.enabledStates]); syncStateControls(); loadEnabledStates(fit)}
...
function nearMeVisibleStateCodes(){
  ...
  const visible=app.shownSites.map(s=>String(s.stateCode||s.state||'').toUpperCase()).filter(Boolean);
  if(visible.length)return sortStateCodes(visible);
  return sortStateCodes(selectedStateCodes());
}
...
function setAllLayers(on){app.restOnlyMode=false;syncRestOnlyToggle();MAP_LAYERS.forEach(l=>on?app.enabledLayers.add(l.key):app.enabledLayers.delete(l.key)); saveLayers(); $('showPendingLayer').checked=app.enabledLayers.has('pending');loadEnabledStates(false)}
...
function stateNearLocation(code,lat,lng,radiusMiles=NEAR_ME_RADIUS_MILES){const b=STATE_BOUNDS[String(code).toUpperCase()];if(!b)return false;if(pointInBounds(lat,lng,b))return true;const pts=[[b[0][0],b[0][1]],[b[0][0],b[1][1]],[b[1][0],b[0][1]],[b[1][0],b[1][1]],[(b[0][0]+b[1][0])/2,(b[0][1]+b[1][1])/2],[lat,Math.max(b[0][1],Math.min(lng,b[1][1]))],[Math.max(b[0][0],Math.min(lat,b[1][0])),lng]];return pts.some(p=>distanceMiles(lat,lng,p[0],p[1])<=radiusMiles)}
...
function showUserMarker(ll){if(app.userMarker)app.userMarker.remove();app.userMarker=L.marker(ll,{icon:L.divIcon({className:'',html:`<span class="map-pin pin-user">${ICONS.dot}</span>`,iconSize:[24,24],iconAnchor:[12,12]})}).addTo(app.map).bindPopup(`You are here — showing roughly ${NEAR_ME_RADIUS_MILES} miles around you`).openPopup()}
...
async function applyNearMeLocation(lat,lng){const ll=[lat,lng];app.localAreaCenter=ll;app.nearMeActive=true;const codes=mappedStatesNearLocation(lat,lng);app.enabledStates=new Set(codes);saveJson(STORE.states,[...app.enabledStates]);syncStateControls();app.sites=[];app.shownSites=[];renderMarkers(false);setLoading(true,`Loading Near Me (${NEAR_ME_RADIUS_MILES} mi)…`);notify(`Loading Near Me within ${NEAR_ME_RADIUS_MILES} miles: ${codes.join(', ')}`,3500);await loadEnabledStates(false);if(app.nearMeActive&&app.localAreaCenter===ll){showUserMarker(ll);fitNearMeRadius(ll);renderMarkers(false);syncStateControls();setLoading(false);}}
function nearMe(){if(!navigator.geolocation)return notify('Location is not available in this browser.');app.nearMeActive=true;app.localAreaCenter=null;app.enabledStates=new Set();saveJson(STORE.states,[]);app.sites=[];app.shownSites=[];renderMarkers(false);syncStateControls();setLoading(true,'Getting your location for Near Me…');notify('Getting your location for Near Me…');navigator.geolocation.getCurrentPosition(pos=>{applyNearMeLocation(pos.coords.latitude,pos.coords.longitude).catch(e=>{console.error(e);setLoading(false);notify('Could not load the nearby map area.')})},()=>{setLoading(false);notify('Could not get your location. Check browser/site location permission.')},{enableHighAccuracy:false,timeout:12000,maximumAge:300000})}
